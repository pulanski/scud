...
// setup formatting
// Select formatting tool for increased code quality via maintained consistent coding styles for multiple developers
//
// setup formatting editorconfig
// setup formatting prettier
// setup formatting --all
//
// help implement DRY patterns into your development workflow so you can focus on the core functionality of your project and the problems you are trying to solve
...
pub fn log_commit_message(commit_type: String, scope: String, subject: String, body: String, breaking_changes: String, refs: String) {
    let commit_message_header =
        " GENERATED COMMIT MESSAGE "
            .on_red();
    let commit_message_note =
        "(colored output omitted from commit message)"
            .green()
            .italic();
    let commit_type =
        commit_type
            .bright_yellow();
    let left_paren = black_open_paren();
    let scope =
        scope
            .green();
    let right_paren = black_close_paren();
    let subject =
        subject
            .bright_yellow();
    let body =
